...
def ask(in_: AskIn):
    ...
    with SessionLocal() as db:
        ...
        for k,v in (bits or {}).items():
            ...
            if isinstance(v,list): base[k]=sorted(list(set((base.get(k,[]) or [])+v)))
            else: base[k]=v
        u.profile = json.dumps(base, ensure_ascii=False)

        inter = Interaction(user_id=u.id, kind="nutrition", question=in_.query, answer=answer, extracted=json.dumps(bits, ensure_ascii=False))
...
def plan(in_: PlanIn):
    ...
    with SessionLocal() as db:
        ...
        plan_md = _openai([{"role":"system","content": SYSTEM},{"role":"user","content": prompt}], temperature=0.2, max_tokens=1600)
        ...
        p = MealPlan(user_id=u.id, week=w, summary=f"Plan nutricional {w}", details=plan_md, kcal_target=kcal)
        db.add(p); db.commit()
...
def log_meal(in_: LogIn):
    with SessionLocal() as db:
        u = db.execute(select(User).where(User.external_id==in_.user_id)).scalar_one_or_none()
        if not u: raise HTTPException(404, "user not found")
        entry = MealLog(user_id=u.id, date=in_.date, meal=in_.meal, food=in_.food,
                        kcal=in_.kcal, protein_g=in_.protein_g, carbs_g=in_.carbs_g, fat_g=in_.fat_g, notes=in_.notes or "")
